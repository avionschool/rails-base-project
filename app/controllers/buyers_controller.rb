class BuyersController < ApplicationController
  ...
  def create
    if BuyerStock.where(user_id: current_user.id).where(ticker: params[:ticker]).exists?
  ...
  def new
    @stock = BrokerStock.find_by_id(params[:id])
    @existing = BuyerStock.where(user_id: current_user.id).where(ticker: @stock.ticker).exists?
